...
contract StrategyETHConvex {
    ...
    function redelivery() internal {
        ...
        if (reward > redeliverynum)
        {
            ...
            if(_crvAmount > 0){
                UniswapRouter(unirouter).swapExactTokensForTokens(_crvAmount,0,swapCRV2ETHRouting,address(this),now.add(1800));
            }
            uint256 _cvxAmount = IERC20(CVX).balanceOf(address(this));
            if(_cvxAmount > 0){
                UniswapRouter(unirouter).swapExactTokensForTokens(_cvxAmount,0,swapCVX2ETHRouting,address(this),now.add(1800));
            }
