...
contract ldoh is BlockableContract {
	...
	uint256 private constant nocashback 	= 28;        	// * 12% + 16% = 28% from deposit
...
// Function 02 - Contribute (Hodl Platform)
    ...
    function HodlTokens(address tokenAddress, uint256 amount) public contractActive {
		...
		uint256 no_cashback 			= div(mul(amount, nocashback), 100); 	// amount * 28 / 100
...
// Function 03 - Claim (Hodl Platform)	
    ...
    function RetireHodl(address tokenAddress, uint256 id) private {
		...
		//1 uint256 CalculateWithdraw 	= div(mul(div(mul(s.amount, s.percentage), 100), timeframe), seconds30days); 
    ...
    function UpdateUserData(address tokenAddress, uint256 id, uint256 newamountbalance, uint256 realAmount) private {
		...
		//2 uint256 tokenreceived 		= add(sub(sub(sub(s.amount, tokenaffiliate), newamountbalance), maxcashback), s.cashbackbalance ) ;		

			uint256 tokenreceived 		= s.amount - tokenaffiliate - newamountbalance - maxcashback + s.cashbackbalance ;

			

			// Cashback = 100 - 12 - 88 - 16 + 16 = 0 ----> No_Cashback 	= 100 - 12 - 72 - 16 + 0 = 1
