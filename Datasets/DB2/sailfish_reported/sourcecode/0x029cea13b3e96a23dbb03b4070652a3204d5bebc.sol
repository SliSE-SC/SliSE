...
contract FeeApprover is OwnableUpgradeSafe {
    ...
    function initialize(
        ...
        _editNoFeeList(msg.sender, true);
    ...
    uint256 public burnPercentX100;
    ...
    function setFeeMultiplier(uint256 _feeMultiplier) public onlyOwner {
        feePercentX100 = _feeMultiplier;
    }

    function setBurnMultiplier(uint256 _burnMultiplier) public onlyOwner {
        burnPercentX100 = _burnMultiplier;
    }
    ...
    function calculateAmountsAfterFee(        
        ...
        uint256 amount
        ) public  returns (uint256 transferToAmount, uint256 transferToFeeDistributorAmount, uint256 transferToBurnAmount) 
        {
            ...
            if(noFeeList[sender]) { // Dont have a fee when corevault is sending, or infinite loop                       // And when pair is sending ( buys are happening, no tax on it)
                ...
                // START - STCP
                transferToBurnAmount = amount.mul(burnPercentX100).div(1000);
                transferToAmount = amount.sub(transferToFeeDistributorAmount).sub(transferToBurnAmount);
                // END - STCP
            }
