...
// File: contracts/HashmaskSwapper.sol
...
contract HashmaskSwapper is ReentrancyGuard {
  ...
  * @dev Take up an existing swap that's a sell. Only swaps the taker's name to the swap creator's name.
  ...
  function takeSwap(uint256 swapId, uint256 takerId, string calldata placeholder1) external nonReentrant {
    ...
    // Both names are set to lowercase here, and the sha256 is compared
  ...
  function getAllOpenSwaps(bool getSwap) public view returns(int128[] memory) {
    ...
    for (uint256 i = 0; i < len; i++) {
      ...
      // If get swap, only get swap ids, everything else is -1
      if (getSwap) {
      ...
      // Otherwise, get only sale ids, everything else is -1
      else {
        if (currSwap.token != address(0)) {
          swapList[i] = int128(id);
        }
