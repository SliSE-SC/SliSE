...
contract RugProofMaster {
    ...
    struct SaleInfo {
      ...
      uint8 liquidityLockPercent; // 20000 = 20%, capped at 100%, intervals of 0.01%
    ...
    function endTokenSale(uint256 contractIndex) external {
        ...
        uint256 hardcapETH = tokenSaleInfo.tokenAmount.mul(10000 - tokenSaleInfo.liquidityLockPercent).div(10000);
        ...
        if(tokenSaleInfo.totalEth >= tokenSaleInfo.softcap){
          ...
          uint256 saleEthToLock = tokenSaleInfo.totalEth.mul(tokenSaleInfo.liquidityLockPercent).div(10000);
