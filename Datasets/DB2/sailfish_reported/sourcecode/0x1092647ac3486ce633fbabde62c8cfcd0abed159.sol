...
contract ldoh is BlockableContract {
    ...
    */
    ...
    mapping(address => string) public profileHashed; // User Profile

    uint256 public hodlingTime;

    uint256 public allTimeHighPrice;

	uint256 private constant percent = 300;        // new 3.0%

	uint256 private constant affiliate = 12;        // new 

    uint256 private constant seconds30days = 2592000;        // new 
    ...
    struct Safe {
		...
		uint256 amountbalance; //new

		uint256 lasttime; //new
    ...
    */
    ...
    mapping(address => uint256) public _totalSaved; // Token Balance count
    ...
    */
    ...
    function HodlTokens(address tokenAddress, uint256 amount) public contractActive {
        ...
        require(token.transferFrom(msg.sender, address(this), amount));
		    ...
		    uint256 affiliatecomission = mul(amount, affiliate) / 100; // new 

            uint256 amountbalance = sub(amount, affiliatecomission); // new 

		      

        _userSafes[msg.sender].push(_currentIndex);

        _safes[_currentIndex] = Safe(_currentIndex, amount, amountbalance, now, now + hodlingTime, msg.sender, tokenAddress, token.symbol()); //new
    ...
    */
    ...
    function RetireHodl(address tokenAddress, uint256 id) private {
...

        {
			...
			uint256 timeframe = sub(now, s.lasttime);			// new 

			uint256 prewithdraw = mul(s.amount, percent) / 100; // new 

			uint256 realAmount = mul(prewithdraw, timeframe) / seconds30days; // new 

			

			uint256 newamountbalance = sub(s.amountbalance, realAmount);	 // new 
        ...
        s.amountbalance = newamountbalance;  // new 



        _totalSaved[s.tokenAddress] = sub(_totalSaved[s.tokenAddress], s.amount); // new 
