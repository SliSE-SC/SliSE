 ...
 * @title MoonStaking
 */
contract MoonStaking is Ownable, Pausable, SupporterRole, ManagerRole, ReentrancyGuard {
    ...
    /**
     * @param _moon The MOON token address.
     */
    constructor(address _moon, uint256 _rate, uint256 _adapter, uint256 _apy, uint256 _apyAdapter) public {
        _vault = new __unstable__TokenVault();
        moon = IERC20(_moon);
        pools[_moon].rate = _rate;
        pools[_moon].adapter = _adapter;
        pools[_moon].apy = _apy;
        pools[_moon].apyAdapter = _apyAdapter;

        // default pools
        pools[0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599].rate = 1610300000000;
        pools[0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599].adapter = 1;
        pools[0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599].apy = 329;
        pools[0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599].apyAdapter = 1000;

        pools[0x6B175474E89094C44Da98b954EedeAC495271d0F].rate = 1;
        pools[0x6B175474E89094C44Da98b954EedeAC495271d0F].adapter = 1;
        pools[0x6B175474E89094C44Da98b954EedeAC495271d0F].apy = 347;
        pools[0x6B175474E89094C44Da98b954EedeAC495271d0F].apyAdapter = 1000;

        pools[0x0000000000085d4780B73119b644AE5ecd22b376].rate = 1;
        pools[0x0000000000085d4780B73119b644AE5ecd22b376].adapter = 1;
        pools[0x0000000000085d4780B73119b644AE5ecd22b376].apy = 352;
        pools[0x0000000000085d4780B73119b644AE5ecd22b376].apyAdapter = 1000;

        pools[0x4Fabb145d64652a948d72533023f6E7A623C7C53].rate = 1;
        pools[0x4Fabb145d64652a948d72533023f6E7A623C7C53].adapter = 1;
        pools[0x4Fabb145d64652a948d72533023f6E7A623C7C53].apy = 314;
        pools[0x4Fabb145d64652a948d72533023f6E7A623C7C53].apyAdapter = 1000;

        pools[0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48].rate = 1000000;
        pools[0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48].adapter = 1;
        pools[0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48].apy = 368;
        pools[0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48].apyAdapter = 1000;

        pools[0xdAC17F958D2ee523a2206206994597C13D831ec7].rate = 1000000;
        pools[0xdAC17F958D2ee523a2206206994597C13D831ec7].adapter = 1;
        pools[0xdAC17F958D2ee523a2206206994597C13D831ec7].apy = 349;
        pools[0xdAC17F958D2ee523a2206206994597C13D831ec7].apyAdapter = 1000;
    }
    ...
    * @return True when successful
    */
    function updateEpoolRate(uint256 _rate, uint256 _adapter, uint256 _apy, uint256 _apyAdapter)
        public onlySupporter returns (bool) {
        _ePool.rate = _rate;
        _ePool.adapter = _adapter;
        _ePool.apy = _apy;
        _ePool.apyAdapter = _apyAdapter;
        return true;
    }
    ...
    */
    function poolInfo(address poolAddress) public view returns (
        ...
        return (
            ...
            sp.staked,
            sp.lastRewardTime,
            _getMoonPerYear(pool, sp).div(365),
            sp.earned,
            _getEarned(pool, sp)
        );
    }
    ...
    */
    function poolEInfo() public view returns (
        ...
        UserSp storage sp = users[_msgSender()].ePool;
        return (
            _ePool.rate,
            _ePool.adapter,
            _ePool.totalStaked,
            ...
            sp.staked,
            sp.lastRewardTime,
            _getMoonPerYear(_ePool, sp).div(365),
            sp.earned,
            _getEarned(_ePool, sp)
        );
    }
     ...
     */
    function stakeE() public nonReentrant payable returns (bool) {
        uint256 _value = msg.value;
        require(_value != 0, "Zero amount");
        address(_vault).transfer(_value);
        UserSp storage ePool = users[_msgSender()].ePool;
        ePool.earned = _getEarned(_ePool, ePool);
        ePool.lastRewardTime = block.timestamp;
        ePool.staked = ePool.staked.add(_value);
        _ePool.totalStaked = _ePool.totalStaked.add(_value);
        return true;
    }
    ...
    function withdrawTokenPool(address token) public whenNotPaused nonReentrant returns (bool) {
        ...
        _vault.transferToken(moon, _msgSender(),  _getEarned(pool, tokenStakingPool));
    ...
    function withdrawEPool() public whenNotPaused nonReentrant returns (bool) {
        ...
        _vault.transferToken(moon, _msgSender(),  _getEarned(_ePool, eStakingPool));
     ...
     */
    function claimMoonInTpool(address token) public whenNotPaused nonReentrant returns (bool) {
        require(isPoolAvailable(token), "Pool is not available");
        UserSp storage tokenStakingPool = users[_msgSender()].tokenPools[token];
        require(tokenStakingPool.staked > 0 || tokenStakingPool.earned > 0, "Not available");
        Pool storage pool = pools[token];
     ...
     * @dev Claim earned Moon.
     */
    function claimMoonInEpool() public whenNotPaused nonReentrant returns (bool) {
        UserSp storage eStakingPool = users[_msgSender()].ePool;
        require(eStakingPool.staked > 0 || eStakingPool.earned > 0, "Not available");
        _vault.transferToken(moon, _msgSender(), _getEarned(_ePool, eStakingPool));
        eStakingPool.lastRewardTime = block.timestamp;
    ...
    /**
     * @dev Calculate moon reward per year.
     */
    function _getMoonPerYear(Pool memory pool, UserSp memory stakingPool) internal view returns (uint256) {
        Pool storage mPool = pools[address(moon)];
        return (
                stakingPool.staked
                .mul(pool.rate)
                .mul(pool.apy)
    ...
    function _getEarned(Pool memory pool, UserSp memory stakingPool) internal view returns (uint256) {
        uint256 moonPerSec = _getMoonPerYear(pool, stakingPool).div(31536000);
        uint256 lastRewardTime = stakingPool.lastRewardTime == 0 ? block.timestamp : stakingPool.lastRewardTime;
        return block.timestamp.sub(lastRewardTime).mul(moonPerSec).add(stakingPool.earned);
    }
}