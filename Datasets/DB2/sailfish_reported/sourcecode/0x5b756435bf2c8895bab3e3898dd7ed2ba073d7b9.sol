...
contract KyberBancorReserve is KyberReserveInterface, Withdrawable, Utils2 {
    ...
    function getBancorRatePlusPremiumForEthQty(uint ethQty) public view returns(uint, uint) {
        ...
        if(ethQty > 0) buyRate = calcRateFromQty(ethQty, valueAfterAddingPremium(tokenReturn, buyPremiumInBps), getDecimals(ETH_TOKEN_ADDRESS), getDecimals(token));

        if(tokenReturn > 0) sellRate = calcRateFromQty(tokenReturn, valueAfterAddingPremium(ethReturn, sellPremiumInBps), getDecimals(token), getDecimals(ETH_TOKEN_ADDRESS));
