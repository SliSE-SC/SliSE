...
contract _50Win {
    ...
    function takeBet (address with) public payable {
        ...
        uint256 winNum = toss();
        if(winNum==bet.betFor)
            require(send(with, win, tips, bet.referrer), 'Reward failed');//creator win
        else
            require(send(msg.sender, win, tips, bet.referrer), 'Reward failed');//joiner win
