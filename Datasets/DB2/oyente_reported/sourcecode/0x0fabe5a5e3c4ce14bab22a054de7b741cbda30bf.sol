...
contract DMEX_Token {
    ...
    uint256 public shareholderPct = 382e15; // % going to the uniswap contract 38.2%

    address public operationalAddress; // the operational address 
    address public uniswapDmexTokenContract; // the uniswap contract for the DMEX token
    ...
    function extractFeesFromDmex (address token, address dmexContractAddress) public
    {
        ...
        if (token != address(0))
        {
            ...
            ERC20Interface(token).transfer(uniswapTokenContracts[token], availableBalance);

            // swap token for ETH
            ethAmount = UniswapExchangeInterface(uniswapTokenContracts[token]).tokenToEthSwapInput(availableBalance, 1, 1000000000000000000);
        }
        else
        {
        ...
        uint256 shareholderValue = safeMul(ethAmount, shareholderPct) / 1e18;
        uint256 operationalValue = safeSub(ethAmount, shareholderValue);
        
        // deposit shareholder share into the uniswap contract
        uint256 tokens_bought = UniswapExchangeInterface(uniswapDmexTokenContract).ethToTokenSwapInput.value(shareholderValue)(1, 1000000000000000000);
    
        // send the operational value to the operational address
    ...
    function allowUniswap(uint256 tokenAmount)
    {
        ...
        LogUint(5, tokenAmount);
        LogUint(6, allowance[this][uniswapDmexTokenContract]);
    }
