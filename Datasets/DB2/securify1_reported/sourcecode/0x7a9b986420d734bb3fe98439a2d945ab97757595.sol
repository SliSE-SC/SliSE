...
contract MyBancorNetwork is ContractIds {
    ...
    */
...

    {
        ...
        (virtualBalance, weight, isVirtualBalanceEnabled, isSaleEnabled, isSet) = fixGetConnectors(_converter, _connector);
    ...
    */
    ...
    function fixGetConnectors(IBancorConverter _converter, IERC20Token _connector) internal returns(uint256 virtualBalance, uint32 weight, bool isVirtualBalanceEnabled, bool isSaleEnabled, bool isSet) {

        bytes memory ret = new bytes(64);

        bytes memory versionCalldata = abi.encodeWithSelector(_converter.version.selector);
        ...
        if (success) {
            ...
            if (v2 > 0) {
                ...
                (virtualBalance, weight, isVirtualBalanceEnabled, isSaleEnabled, isSet) = _converter.reserves(_connector);
    ...
    function fixGetConverterBalance(IBancorConverter _converter, IERC20Token _connector) internal returns(uint256) {
        ...
        if (success) {
            ...
            if (v2 > 0) {
                ...
                return _converter.getReserveBalance(_connector);
