...
contract LockedResources is Ownable {
  ...
  uint256 private constant batchWidthdraw = 10000 * 10 ** decimals;
  
  uint256 public WithdrawTime = now;
  ...
  function refundAll() onlyOwner public{
    require(now > WithdrawTime + 30 days); 
    WithdrawTime = now;
    token.transfer(owner(), batchWidthdraw);  
  }
