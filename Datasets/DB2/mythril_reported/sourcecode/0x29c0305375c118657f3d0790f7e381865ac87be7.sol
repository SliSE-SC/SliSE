...
//license of Barter Smart Co.,Ltd. Thailand (Applied from MIT)
...
contract BigPoint is ERC20Interface, Owned {
    ...
    function CalWeiToPay(uint _ordertoPay, uint _ShareWei)public onlyOwner view returns(address payable, uint){
    ...
    function PayShareInvividualinWei(address payable _n, uint _toPayinWei)payable public onlyOwner returns(bool){
        ...
        uint individualAmount = balanceOf(_n).mul(_toPayinWei).div(_totalSupply);
    ...
    uint public repeated;
    function CheckRepeatedShare(address payable _check)public returns(uint){
        for(uint y=0; y<i; y++){
            if (_check==ShareAddress[y]){
                repeated = 1;
