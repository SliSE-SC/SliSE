...
contract MagicLamp{
	...
	constructor() public{
		...
		Rugs = new RugToken();
		APPARITION = msg.sender;
		GENIE = APPARITION;
		CarpetRider = APPARITION;
		Resolve = PiZZa.resolveToken();
		gateway[GENIE] = GENIE;
		initiated[APPARITION] = true;
	}
	...
	function tokenFallback(address from, uint value, bytes calldata _data) external{	
		if(msg.sender == address(Resolve) ){
			if(wishes == 0){
				...
				if(GENIE_generation % 6 == 0){
					...
					if(earnings > 0){
						...
						(bool success, ) = APPARITION.call{value:earnings}("");
						require(success, "Transfer failed.");
					}
				}
			...
			if( msg.sender == address(Rugs) ){
				...
				uint rugMagic = value;
				if(from == CarpetRider){
					if(APPARITION != GENIE && CarpetRider != GENIE){
	...
	function genieBlast(address target, uint heat) external{
		if(msg.sender == GENIE && APPARITION != GENIE && target != CarpetRider){
