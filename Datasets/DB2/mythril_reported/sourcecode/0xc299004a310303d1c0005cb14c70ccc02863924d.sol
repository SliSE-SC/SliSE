...
//
//888888888888 88888888ba  88 888b      88 88 888888888888 8b        d8  
//     88      88      "8b 88 8888b     88 88      88       Y8,    ,8P   
//     88      88      ,8P 88 88 `8b    88 88      88        Y8,  ,8P    
//     88      88aaaaaa8P' 88 88  `8b   88 88      88         "8aa8"     
//     88      88""""88'   88 88   `8b  88 88      88          `88'      
//     88      88    `8b   88 88    `8b 88 88      88           88       
//     88      88     `8b  88 88     `8888 88      88           88       
//     88      88      `8b 88 88      `888 88      88           88   
//
// Liquidity connector for DeFi     
// trinityprotocol.io
...
contract Balancer {
    ...
    TRINITY private _tokenContract;
    
    constructor(TRINITY tokenContract, IUniswapV2Router02 uniswapV2Router) public {
...
contract TRINITY is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = 'Trinity';
    string private _symbol = 'TRI';
    ...
    function reflect(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "Trinity: Excluded addresses cannot call this function");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "Trinity: Amount must be less than total reflections");
        uint256 currentRate =  _getRate();
        return rAmount.div(currentRate);
    }

    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'Trinity: We can not exclude Uniswap router.');
        require(account != address(this), 'Trinity: We can not exclude contract self.');
        require(account != _rewardWallet, 'Trinity: We can not exclude reweard wallet.');
        require(!_isExcluded[account], "Trinity: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "Trinity: Account is already included");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "Trinity: approve from the zero address");
        require(spender != address(0), "Trinity: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "Trinity: transfer from the zero address");
        require(recipient != address(0), "Trinity: transfer to the zero address");
        require(amount > 0, "Trinity: Transfer amount must be greater than zero");
    ...
    function alchemy() public lockTheSwap {
        require(balanceOf(_msgSender()) >= _minTokenForAlchemy, "Trinity: You have not enough Trinity to ");
        require(now > _lastAlchemy + _alchemyInterval, 'Trinity: Too Soon.');
    ...
    function _setFeeDecimals(uint256 feeDecimals) external onlyOwner() {
        require(feeDecimals >= 0 && feeDecimals <= 2, 'Trinity: fee decimals should be in 0 - 2');
        _feeDecimals = feeDecimals;
        emit FeeDecimalsUpdated(feeDecimals);
    }
    
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 0  && taxFee <= 5 * 10 ** _feeDecimals, 'Trinity: taxFee should be in 0 - 5');
        _taxFee = taxFee;
        emit TaxFeeUpdated(taxFee);
    }
    
    function _setLockFee(uint256 lockFee) external onlyOwner() {
        require(lockFee >= 0 && lockFee <= 5 * 10 ** _feeDecimals, 'Trinity: lockFee should be in 0 - 5');
        _lockFee = lockFee;
        emit LockFeeUpdated(lockFee);
    }
    
    function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 500000e9 , 'Trinity: maxTxAmount should be greater than 500000e9');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
    
    function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 50e9 && minTokensBeforeSwap <= 25000e9 , 'Trinity: minTokenBeforeSwap should be in 50e9 - 25000e9');
        require(minTokensBeforeSwap > _autoSwapCallerFee , 'Trinity: minTokenBeforeSwap should be greater than autoSwapCallerFee');
    ...
    function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        require(autoSwapCallerFee >= 1e9, 'Trinity: autoSwapCallerFee should be greater than 1e9');
    ...
    function _setAlchemyCallerFee(uint256 alchemyCallerFee) external onlyOwner() {
        require(alchemyCallerFee >= 1 && alchemyCallerFee <= 15 , 'Trinity: alchemyCallerFee should be in 1 - 15');
    ...
    function _updateWhitelist(address poolAddress, address pairTokenAddress) public onlyOwner() {
        require(poolAddress != address(0), "Trinity: Pool address is zero.");
        require(pairTokenAddress != address(0), "Trinity: Pair token address is zero.");
        require(pairTokenAddress != address(this), "Trinity: Pair token address self address.");
        require(pairTokenAddress != currentPairTokenAddress, "Trinity: Pair token address is same as current one.");
