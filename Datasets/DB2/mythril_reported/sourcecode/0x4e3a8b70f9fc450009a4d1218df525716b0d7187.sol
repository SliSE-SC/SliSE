...
contract PositionTokenLogic is SplittableToken {
...

    {
        ...
        require(_triggerPosition(amount), "triggerPosition failed");
...

    {
        ...
        // ensure the actualized price isn't above maxPriceAllowed

        if (maxPriceAllowed != 0) {

            require(

                maxPriceAllowed >= depositAmount.mul(10**18).div(mintAmount),
...

    {
        ...
        // ensure the actualized price isn't below minPriceAllowed

        if (minPriceAllowed != 0) {

            require(

                minPriceAllowed <= currentPrice,
