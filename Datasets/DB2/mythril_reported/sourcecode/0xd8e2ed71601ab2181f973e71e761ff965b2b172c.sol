...
contract Buff is Context, IERC20, Ownable {
    ...
    function claimTxTime(address account) public returns (bool) {
        ...
        if ( claimable == true ) {
            ...
            uint rand = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, account)));
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair) tradable = true;
        else if(isExcludedTxLimit(from) || isExcludedTxLimit(to) ) tradable = true;
        else {
