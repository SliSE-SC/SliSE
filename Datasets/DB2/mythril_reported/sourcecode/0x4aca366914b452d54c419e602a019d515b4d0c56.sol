...
/**
 *Submitted for verification at BscScan.com on 2020-09-30
*/
...
contract IRewardDistributionRecipient is Ownable {
    ...
    function notifyRewardAmount(uint amount) public;
...
contract USDTPool is IRewardDistributionRecipient {
    ...
    function notifyRewardAmount(uint amount) public nonReentrant onlyRewardDistribution{
        ...
        _rewardPerToken = _rewardPerToken.add(amount.mul(1e6)/_totalSupply);
    }
    ...
    function stake(uint256 amount) public {
        ...
        require(!stopinvest, "invest stopped");
    ...
    function withdraw(uint256 amount) public {
        ...
        require(!lock, "money is lock");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        userRewardPaid[msg.sender] = userRewardPaid[msg.sender].sub(amount.mul(_rewardPerToken)/1e6);
    ...
    function exit() external {
     ...
     function earned(address account) public view returns (uint256) {
        return
            (_balances[account].mul(_rewardPerToken)/1e6).sub(userRewardPaid[account]);
    }
    ...
    function getReward() public nonReentrant {
        ...
        userRewardPaid[msg.sender] = _balances[msg.sender].mul(_rewardPerToken)/1e6;
    ...
    function transferAnyERC20Token(address tokenAddress, uint tokens, address who) public onlyOwner returns (bool success) {
        return IERC20(tokenAddress).transfer(who, tokens);
    }
