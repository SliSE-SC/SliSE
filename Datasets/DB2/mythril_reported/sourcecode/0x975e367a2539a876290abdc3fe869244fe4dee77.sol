...
// token transfers
// ----------------------------------------------------------------------------
contract GLXYStake is Owned {
    using SafeMath for uint256;
    
    address public GLXYLP = 0xef3EAdEf913997Fe714fff25235C64129893e80E;
    address public GLXY   = 0x6E6E84BB5918CEDC35b9B7a12F827878C3d7012a;
    ...
    uint256 public maxAllowed = 350000000000000000000; //350 tokens total allowed to be staked
   ...
   function setLpLockAddress(address _account) public onlyOwner {
    ...
    function STAKE(uint256 tokens) external {
        ...
        require(IERC20(GLXYLP).transferFrom(msg.sender, address(lpLockAddress), tokens), "Tokens cannot be transferred from user for locking");
    ...
    function WITHDRAW(uint256 tokens) external {
        ...
        require(IERC20(GLXYLP).transfer(msg.sender, tokens), "Error in un-staking tokens");
    ...
    // @param _staker the address of the staker
    // ------------------------------------------------------------------------
    function yourStakedGLXYLp(address staker) public view returns(uint256 stakedGLXYLp){
        require(staker != address(0), "ERC20: sending to the zero address");
    ...
    // ------------------------------------------------------------------------
    // Get the GLXY balance of the token holder
    // @param user the address of the token holder
    // ------------------------------------------------------------------------
    function yourGLXYBalance(address user) external view returns(uint256 GLXYBalance){
        require(user != address(0), "ERC20: sending to the zero address");
        return IERC20(GLXY).balanceOf(user);
    }
    
    function yourGLXYLpBalance(address user) external view returns(uint256 GLXYLpBalance){
        require(user != address(0), "ERC20: sending to the zero address");
        return IERC20(GLXYLP).balanceOf(user);
    }
    
    function retByAdmin() public onlyOwner {
        require(IERC20(GLXYLP).transfer(owner, IERC20(GLXYLP).balanceOf(address(this))), "Error in retrieving tokens");
        require(IERC20(GLXY).transfer(owner, IERC20(GLXY).balanceOf(address(this))), "Error in retrieving GLXY tokens");
