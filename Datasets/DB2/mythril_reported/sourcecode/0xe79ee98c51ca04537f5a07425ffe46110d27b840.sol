...
/*! sp500crypto.sol | (c) 2019 Develop by BelovITLab LLC (smartcontract.ru), author @stupidlovejoy | License: MIT */
...
contract Token is MintableToken, BurnableToken, Withdrawable, Pausable {
    mapping(address => bool) blockeds;
    
    uint public commission = 0;
    uint public maxFee = 1e8;

    modifier whenNotBlocked(address addr) {
        require(!blockeds[addr], "Blocked: block");
        _;
    }

    constructor() StandardToken("Russian Goverment Bonds", "RURB", 6) public {
        
    }
    
    function transfer(address to, uint256 value) public whenNotPaused whenNotBlocked(msg.sender) returns (bool) {
        if(commission > 0) {
            uint fee = value.mul(commission).div(10000);
    ...
    function transferFrom(address from, address to, uint256 value) public whenNotPaused whenNotBlocked(from) returns (bool) {
        if(commission > 0) {
            uint fee = value.mul(commission).div(10000);
            if(fee > maxFee) fee = maxFee;
    ...
    function blockAddr(address addr) public onlyOwner {
        blockeds[addr] = true;
    }

    function unblockAddr(address addr) public onlyOwner {
        blockeds[addr] = false;
    }
    ...
    function setMaxFee(uint value) public onlyOwner {
        maxFee = value;
    }
}