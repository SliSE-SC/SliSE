...
contract TokenGeyser is IStaking, Ownable {
    ...
    function _stakeFor(address staker, address beneficiary, uint256 amount) private {
        require(_paused == 0, 'TokenGeyser: paused. contract needs to be resumed');
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        require(_paused == 0, 'TokenGeyser: paused. contract needs to be resumed to allow reward transfer');
    ...
    /**
     * @dev Contract pause resume helpers
     */
    ...
    /**
     * @dev Transfers ownership of the contracts 3 pools to a new account (`newOwner`).
     * Can only be called by the current owner. Used to rescue funds
     */
    function transferOwnershipPools(address newOwner) public onlyOwner {
        require(_paused == 1, 'TokenGeyser: needs to be paused');

        _unlockedPool.transferOwnership(newOwner);
        _lockedPool.transferOwnership(newOwner);
    }
}