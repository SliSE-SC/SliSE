...
contract Balancer {
    ...
    MRINITY private _tokenContract;
    
    constructor(MRINITY tokenContract, IUniswapV2Router02 uniswapV2Router) public {
...
contract MRINITY is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = 'Mrinity';
    string private _symbol = 'MRINITY';
    ...
    uint256 private _magicCallerFee = 5;
    uint256 private _minTokenForMagic = 1000e9;
    uint256 private _lastMagic;
    uint256 private _magicInterval = 1 hours;
    ...
    event MagicCallerFeeUpdated(uint256 rebalanceCallerFee);
    ...
    function reflect(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "Mrinity: Excluded addresses cannot call this function");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "Mrinity: Amount must be less than total reflections");
        uint256 currentRate =  _getRate();
        return rAmount.div(currentRate);
    }

    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'Mrinity: We can not exclude Uniswap router.');
        require(account != address(this), 'Mrinity: We can not exclude contract self.');
        require(account != _rewardWallet, 'Mrinity: We can not exclude reweard wallet.');
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "Mrinity: approve from the zero address");
        require(spender != address(0), "Mrinity: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "Mrinity: transfer from the zero address");
        require(recipient != address(0), "Mrinity: transfer to the zero address");
        require(amount > 0, "Mrinity: Transfer amount must be greater than zero");
        
        if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {
            require(amount <= _maxTxAmount, "Mrinity: Transfer amount exceeds the maxTxAmount.");
            if((_msgSender() == currentPoolAddress || _msgSender() == address(_uniswapV2Router)) && !tradingEnabled)
                require(false, "Mrinity: trading is disabled.");
        }
    ...
    function magic() public lockTheSwap {
        require(balanceOf(_msgSender()) >= _minTokenForMagic, "Mrinity: You do not have enough Mrinity to ");
        require(now > _lastMagic + _magicInterval, 'Mrinity: Too Soon.');
        ...
        uint256 tRewardForCaller = tNewTokenBalance.mul(_magicCallerFee).div(100);
    ...
    function getMagicCallerFee() public view returns(uint256) {
        return _magicCallerFee;
    }
    
    function getMinTokenForMagic() public view returns(uint256) {
        return _minTokenForMagic;
    }
    
    function getLastMagic() public view returns(uint256) {
        return _lastMagic;
    }
    
    function getMagicInterval() public view returns(uint256) {
        return _magicInterval;
    }
    
    function _setFeeDecimals(uint256 feeDecimals) external onlyOwner() {
        require(feeDecimals >= 0 && feeDecimals <= 2, 'Mrinity: fee decimals should be in 0 - 2');
        _feeDecimals = feeDecimals;
        emit FeeDecimalsUpdated(feeDecimals);
    }
    
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 0  && taxFee <= 5 * 10 ** _feeDecimals, 'Mrinity: taxFee should be in 0 - 5');
        _taxFee = taxFee;
        emit TaxFeeUpdated(taxFee);
    }
    
    function _setLockFee(uint256 lockFee) external onlyOwner() {
        require(lockFee >= 0 && lockFee <= 5 * 10 ** _feeDecimals, 'Mrinity: lockFee should be in 0 - 5');
        _lockFee = lockFee;
        emit LockFeeUpdated(lockFee);
    }
    
    function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 500000e9 , 'Mrinity: maxTxAmount should be greater than 500000e9');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
    
    function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 50e9 && minTokensBeforeSwap <= 25000e9 , 'Mrinity: minTokenBeforeSwap should be in 50e9 - 25000e9');
        require(minTokensBeforeSwap > _autoSwapCallerFee , 'Mrinity: minTokenBeforeSwap should be greater than autoSwapCallerFee');
    ...
    function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        require(autoSwapCallerFee >= 1e9, 'Mrinity: autoSwapCallerFee should be greater than 1e9');
    ...
    function _setLiquidityRemoveFee(uint256 liquidityRemoveFee) external onlyOwner() {
        require(liquidityRemoveFee >= 1 && liquidityRemoveFee <= 10 , 'Mrinity: liquidityRemoveFee should be in 1 - 10');
        _liquidityRemoveFee = liquidityRemoveFee;
        emit LiquidityRemoveFeeUpdated(liquidityRemoveFee);
    }
    
    function _setMagicCallerFee(uint256 magicCallerFee) external onlyOwner() {
        require(magicCallerFee >= 1 && magicCallerFee <= 15 , 'Mrinity: magicCallerFee should be in 1 - 15');
        _magicCallerFee = magicCallerFee;
        emit MagicCallerFeeUpdated(magicCallerFee);
    }
    
    function _setMinTokenForMagic(uint256 minTokenForMagic) external onlyOwner() {
        _minTokenForMagic = minTokenForMagic;
        emit MinTokenForMagicUpdated(minTokenForMagic);
    }
    
    function _setMagicInterval(uint256 magicInterval) external onlyOwner() {
        _magicInterval = magicInterval;
        emit MagicIntervalUpdated(magicInterval);
    }
    ...
    function _updateWhitelist(address poolAddress, address pairTokenAddress) public onlyOwner() {
        require(poolAddress != address(0), "Mrinity: Pool address is zero.");
        require(pairTokenAddress != address(0), "Mrinity: Pair token address is zero.");
        require(pairTokenAddress != address(this), "Mrinity: Pair token address self address.");
        require(pairTokenAddress != currentPairTokenAddress, "Mrinity: Pair token address is same as current one.");
