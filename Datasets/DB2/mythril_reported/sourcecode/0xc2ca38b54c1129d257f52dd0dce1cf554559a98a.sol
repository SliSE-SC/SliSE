...
contract TRIDENT is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "TRIDENT";
    ...
    uint256 internal _tokenTotal = 10_000_000e18;
    ...
    uint256 public maxTxAmount = 2000e18;
    uint256 public minTokensBeforeSwap = 10_000e18;
    uint256 public minTokenBeforeReward = 10e18;
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify) {
            bool swap = true;
            if(now > lastRebalance + rebalanceInterval && rebalanceEnalbed){
    ...
    function rewardLiquidityProviders() private {
        ...
        if(tokenBalance > minTokenBeforeReward) {
            ...
            _reflectionBalance[uniswapV2Pair] = _reflectionBalance[uniswapV2Pair].add(rewardAmount);
            _reflectionBalance[rewardWallet] = _reflectionBalance[rewardWallet].sub(rewardAmount);
            emit Transfer(rewardWallet, uniswapV2Pair, tokenBalance);
            IUniswapV2Pair(uniswapV2Pair).sync();
        }
    }
    
    function rebalance() public {
        require(now > lastRebalance + rebalanceInterval && balanceOf(_msgSender()) >= 100e9, 'Mrinity: Not allowed Rebalance.');
    ...
    function _rebalance(uint256 lpBalance) private lockTheSwap {
        ...
        //uniswap don't allow for a token to by itself, so we have to use an external account, which in this case is called the balancer
