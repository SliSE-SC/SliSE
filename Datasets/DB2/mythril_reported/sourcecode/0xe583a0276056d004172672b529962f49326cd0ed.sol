...
contract OMX1SUtil is ERC20{
    ...
    address omx1SAddr=0xAE1b72E9C2e06Fd3685c55f90D40CE804aAdDb95;
    uint omx1SDecimals=8;
   ...
   function settle(uint _startIndex,uint _count)public onlyOwner () {
       OMXToken omx1 =OMXToken(omx1SAddr);
        ERC20 token =ERC20(omfAddr);
        uint256 amount;
        for(uint i=_startIndex;i<(_startIndex+_count)&&i<omx1.getCount();i++){
            uint balance=omx1.balanceOf(omx1.users(i))/(1*10**omx1SDecimals);
            if(balance>=1){
