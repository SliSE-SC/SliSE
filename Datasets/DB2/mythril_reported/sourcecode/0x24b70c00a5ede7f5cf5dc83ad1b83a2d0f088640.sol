...
contract TradeEth2DAI {
    ...
    uint constant  MIN_TAKE_AMOUNT_DAI = 40;
    ...
    function findBestOffers(bool isEthToDai, uint srcAmount, function (bool, uint, uint, uint) view returns (uint, uint, uint) calcLim) 
        internal view
        returns(uint totalDestAmount, uint numTaken, uint numTraversed, OfferDescriptor [] memory offers, uint maxTakes, uint maxTraverse, uint minAmount) 
    {
        ...
        (maxTakes, maxTraverse, minAmount) = calcLim(isEthToDai, offers[0].payAmount, offers[0].buyAmount, srcAmount); 
        uint thisOffer;
        numTraversed = 1;
