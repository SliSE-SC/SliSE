...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    uint256 public constant initialSupply = 10000e18; // 10k Total Supply.NO DEV MINT FUNCTION.NO TEAM.NO ICO.NO VC
    ...
    string public liquidityGenerationParticipationAgreement = "I agree that the developers and affiliated parties of the HARDCORE team are not responsible for your funds";
    ...
    function liquidityGenerationOngoing()
        ...
        returns (bool)
    {
        console.log("1 days since start is", contractStartTimestamp.add(1 days), "Time now is", block.timestamp);
        console.log("liquidity generation ongoing", contractStartTimestamp.add(1 days) < block.timestamp);
        return contractStartTimestamp.add(1 days) > block.timestamp;
    }
    ...
    function addLiquidityToUniswapHDCORExWETHPair() public {
        require(liquidityGenerationOngoing() == false, "HDCORE LGE ongoing");
        require(LPGenerationCompleted == false, "HDCORE LGE already finished");
    ...
    function claimLPTokens() public {
        ...
        require(ethContributed[msg.sender] > 0 , "Nothing to claim, move along Hardcore");
        IUniswapV2Pair pair = IUniswapV2Pair(tokenUniswapPair);
    ...
    function _transfer(
        ...
        if(transferToFeeDistributorAmount > 0 && feeDistributor != address(0)){
            ...
            if(feeDistributor != address(0)){
                IHdcoreVault(feeDistributor).addPendingRewards(transferToFeeDistributorAmount);
            }
        }
    }
...
contract HDCORE is NBUNIERC20 {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "HDCORE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "HDCORE::delegateBySig: invalid nonce");
        require(now <= expiry, "HDCORE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "HDCORE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying HDCORE tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "HDCORE::_writeCheckpoint: block number exceeds 32 bits");
