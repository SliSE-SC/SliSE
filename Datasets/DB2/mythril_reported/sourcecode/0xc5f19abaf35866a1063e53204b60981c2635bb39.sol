...
contract ZEREUS is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Zereus Protocol";
    string private _symbol = "ZER";
    ...
    uint256 internal _tokenTotal = 50_000e9;
    ...
    function _transfer(
        ...
        //@dev check if 6 hours has passed since lastTaxIncreasedTime and increase the tax fee by 1%, limit of tax fee increment is set to 4%
        if(lastTaxIncreasedTime != 0 && now > lastTaxIncreasedTime + 6 hours && _taxFee < 4 && _liquidityFee < 4){
            _taxFee += 100;
            _liquidityFee += 100;
