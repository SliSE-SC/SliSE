...
contract PriceOracle is Operators {

    // How much Tokens you get for 1 ETH, multiplied by 10^18
    uint256 public ETHPrice = 1000000000000000000000;

    event PriceChanged(uint256 newPriceInToken);

    function updatePrice(uint256 _newPriceInEth) external onlyOperator {
        require(_newPriceInEth > 0, "Illegal price");
        ETHPrice = _newPriceInEth;
        emit PriceChanged(tokenPrice());
    }

    function updatePriceInTokens(uint256 _newPriceInTokens) external onlyOperator {
        require(_newPriceInTokens > 0, "Illegal price");
        uint _newPriceInEth = 1 ether * 1 ether / _newPriceInTokens;
        require(_newPriceInEth > 0, "Illegal price");
        ETHPrice = _newPriceInEth;
        emit PriceChanged(_newPriceInTokens);
    }

    function tokenPrice() public view returns (uint) {
        return 1 ether * 1 ether / ETHPrice;
    }
}