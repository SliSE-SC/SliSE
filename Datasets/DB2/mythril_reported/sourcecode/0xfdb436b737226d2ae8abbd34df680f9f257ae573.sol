...
{	
    ...
    constructor(uint256 initialSupply, string tokenName, string tokenSymbol, uint8 decimalUnits) public	{
		...
		decimals = decimalUnits;							// Set the decimal units of token
...
contract SEVENS is owned, TokenERC20{
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        require(!frozenAccount[_from], "7s account is frozen"); // Check if sender is frozen

        require(!frozenAccount[_to], "7s account is frozen");  	// Check if recipient is frozen
