...
contract BitSafe is Context, IERC20, Ownable {
    ...
    uint256 private constant RATE_PRECISION = 10 ** 5;
    ...
    constructor() public {
        ...
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F); // Pancake Router V1 for BSC
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // Pancake Router V2 for BSC
    ...
    function distribute(int256 supplyDelta)
        onlyOwner
        external
    {
        if (supplyDelta == 0) {
        ...
        uint256 rate = uSupplyDelta.mul(RATE_PRECISION).div(_tokenTotal);
        uint256 multiplier;
        ...
        if (supplyDelta < 0) {
            _tokenTotal = _tokenTotal.sub(uSupplyDelta);
        } else {
            _tokenTotal = _tokenTotal.add(uSupplyDelta);
        }
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if(_tokenBalance[_excluded[i]] > 0) {
                _tokenBalance[_excluded[i]] = _tokenBalance[_excluded[i]].mul(multiplier).div(RATE_PRECISION);
            }
        }
    ...
    function _transfer(
        ...
        require(maxTxAmount >= amount, "Max Transfer Limit Exceeds!");
