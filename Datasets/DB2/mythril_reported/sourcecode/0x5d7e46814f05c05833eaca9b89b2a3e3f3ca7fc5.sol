...
contract TokenSale {
    ...
    function buy() public payable returns (uint256) {
        ...
        require(IERC20Token(token).transfer(msg.sender, actual));
    ...
    function finishTokenSale() external onlyOwner onlyFinished {
        ...
        require(IERC20Token(token).transfer(msg.sender, balance));
    }
    
    function finishTokenSale(address payable to) external onlyOwner onlyFinished {
        uint256 balance = availableTokens();
        
        require(IERC20Token(token).transfer(to, balance));
    }
}