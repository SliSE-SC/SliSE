...
contract EDrain is ERC20UpgradeSafe, OwnableUpgradeSafe {
    ...
    uint256 public rewardInterval = 15 minutes;
    uint256 public lastRewardTimestamp = block.timestamp;
    uint256 public rewardAmount = 50_000e18;
    bool public rewardEnabled;
    
    address public uniswapV2Pair;
    
    constructor() public {
        __ERC20_init("EthDrain", "$EDrain");
        _setupDecimals(uint8(DECIMALS));
        __Ownable_init();
        
        _totalSupply = 500_000e18;
        _totalSupplyCap = 10_000_000e18;
        ...
        uniswapV2Pair = IUniswapV2Factory(uniswapRouterV2.factory())
            .createPair(address(this), uniswapRouterV2.WETH());

        excludeAccount(uniswapV2Pair);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(rewardEnabled && lastRewardTimestamp + rewardInterval >= block.timestamp && _totalSupplyCap >= _totalSupply.add(rewardAmount)){
            distributeReward(rewardAmount);
            lastRewardTimestamp += 15 minutes;
            rewardAmount += 50_000e18;
        }
    ...
    function distributeReward(uint256 amount) internal returns (uint256) {
        uint256 rate = amount.mul(RATE_PRECISION).div(_totalSupply);
        uint256 multiplier;
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if(_tOwned[_excluded[i]] > 0) {
                uint256 newBalance = _tOwned[_excluded[i]].mul(multiplier).div(RATE_PRECISION);
                if(newBalance > _tOwned[_excluded[i]]){
                    uint256 balanceDelta = newBalance.sub(_tOwned[_excluded[i]]);
                    uint256 rBalanceDelta = balanceDelta.mul(currentRate);
                    _rOwned[_excluded[i]] = _rOwned[_excluded[i]].sub(rBalanceDelta);
                    _rTotal = _rTotal.sub(rBalanceDelta);
                    _tFeeTotal = _tFeeTotal.add(balanceDelta);
                }
            }
        }
