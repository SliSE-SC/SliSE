...
contract DEFISocialLockReserves is Ownable {
    ...
    uint256 relaseTime2 = 240 days;
    uint256 relaseTime3 = 360 days;
    uint256 timing ;
    ...
    function withdraw1() public onlyOwner{
        ...
        uint256 amount = tokens.div(3);   //33% available after 90 days
    ...
    function withdraw2() public onlyOwner{
        ...
        uint256 amount = tokens.div(2); //33% available after 240 days
        require(Token(tokenAddress).transfer(owner, amount), "Could not transfer tokens.");
        tokens = tokens.sub(amount);  
        emit Transferred(owner, amount);
        secondWith = false;
    ...
    function withdraw3() public onlyOwner{
        ...
        require(block.timestamp.sub(timing)>relaseTime3, "Not yet"); //33% available after 360 days
