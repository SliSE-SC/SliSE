...
contract Crowdsale is Whitelist, ReentrancyGuard {
    ...
    IERC20[6] private _stableCoins = [
        IERC20(0x4Fabb145d64652a948d72533023f6E7A623C7C53), // BUSD
        IERC20(0x6B175474E89094C44Da98b954EedeAC495271d0F), // DAI
        IERC20(0x8E870D67F660D95d5be530380D0eC0bd388289E1), // PAX
        IERC20(0x0000000000085d4780B73119b644AE5ecd22b376), // TUSD
    ...
    IDepository private _depository = IDepository(0xD68f1E8B1644358D3448C696E951F20556D7cE5d);
    ...
    function withdraw(address beneficiary) external onlyOwner {
        for (uint8 i = 0; i < _stableCoins.length; i++) {
            IERC20 stableCoin = _stableCoins[i];
            stableCoin.safeTransfer(beneficiary, stableCoin.balanceOf(address(this)));
        }
    }

    function withdraw(IERC20 token, address beneficiary) external onlyOwner {
