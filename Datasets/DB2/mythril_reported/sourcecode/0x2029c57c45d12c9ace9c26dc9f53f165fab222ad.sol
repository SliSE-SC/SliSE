 ...
 */
contract KyraFinance is IERC20, Ownable {
    ...
    constructor () {
        ...
        _symbol = "Kyra";
        _decimals = 18;
        uint256 _maxSupply = 100000;
        
        manager = 0xB7e5a9D9EBF5294002cfCf7524e57b137cA02DdC;
        minimumPurchase = 0.5 ether;
        maximumPurchase = 10 ether;
        tokenPerEther = 72e18;
        saleIsOn = false;
    
        _mintOnce(manager, _maxSupply.mul(10 ** _decimals));
    }
    ...
    */
    ...
    function buyToken() public payable {
        require(saleIsOn, "Token sale is currently closed");
        ...
        require(investors[investor].add(etherAmount) <= maximumPurchase, "Cummulative purchase exceeds maximum allowed");
        investors[investor] = investors[investor].add(etherAmount);
        uint256 totalToken = calculateTotalBuy(etherAmount);
        
        require(balanceOf(address(this)) >= totalToken, "Token balance is not enough for exchange");
        _transfer(address(this), investor, totalToken);
        manager.transfer(etherAmount);
    }
    
    function calculateTotalBuy(uint256 _amount) internal view returns(uint256) {
        return _amount.mul(tokenPerEther).div(1 ether);
    }
    ...
    function setManagerAddress(address payable _address) external onlyOwner {
        manager = _address;
    }
    ...
    function withdrawContractBalance(IERC20 _token) external onlyOwner {
