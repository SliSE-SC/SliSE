...
contract TokenGeyser is IStaking, Ownable {
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        while (sharesLeftToBurn > 0) {
            ...
            if (lastStake.stakingShares <= sharesLeftToBurn) {
                ...
                // **TODO** test to make sure element is popped from the back of the list
                accountStakes.pop();
            } else {
     ...
     * @return The total number of distribution tokens that would be rewarded.
    */
    function rewardPreview(address account) public view returns (uint256) {
    ...
    /**
     * @return The current multiplier coefficient for user [1, 2, 3].
     */
    function getMultiplier(address account) public view returns (uint256) {
        uint256 numerator = 0;
        for (uint256 i = 0; i < _userStakes[account].length; i++) {
            numerator += _userStakes[account][i].stakingShares.mul(block.timestamp.sub(_userStakes[account][i].timestampSec));
        }

        if (numerator == 0)
            return 0;

        uint256 denom = _userTotals[account].stakingShares.mul(bonusPeriodSec);
        if (numerator <= denom) {
            if (numerator <= denom.div(2)) {
                return 1 + numerator.div(denom);
            } else if (numerator > denom.div(2)) {
                return 2 + numerator.div(denom);
            }
        }
        return 3;
    }
    ...
    function getAccruedRewards(address account) external view returns (uint256) {
        // User Accounting
        UserTotals memory totals = _userTotals[account];
        uint256 newUserStakingShareSeconds =
        block.timestamp
        .sub(totals.lastAccountingTimestampSec)
        .mul(totals.stakingShares);
        uint256 totalsStakingShareSeconds =
        totals.stakingShareSeconds
        .add(newUserStakingShareSeconds);

        return (_totalStakingShareSeconds > 0)
        ? totalUnlocked().mul(totalsStakingShareSeconds).mul(getMultiplier(account)).div(_totalStakingShareSeconds)
        : 0;
    }
}