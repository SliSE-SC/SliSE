...
*    The outrageous dev fees from M2D were also fixed.

* 

*    The Mobius BLUE game consists of rounds with guaranteed winners!

*    You buy "shares" (instad of keys) for a given round, and you get returns from investors after you.

*    The share price is constant until the 3 day hard deadline, after which it increases exponentially. 

*    It is guaranteed to finish not much after the hard deadline (and the last investor gets the big jackpot)
...
contract MobiusBLUE is DSMath, DSAuth {
    ...
    uint public constant DEV_FRACTION = WAD / 25;             // 4% goes to devs

    uint public constant DEV_DIVISOR = 25;             // 4% 
    ...
    uint public constant HARD_DEADLINE_DURATION = 1 days; // hard deadline is this much after the round start

    uint public constant SOFT_DEADLINE_DURATION = 1 days; // max soft deadline

    uint public constant TIME_PER_SHARE = 2 minutes; // how much time is added to the soft deadline per share purchased
    ...
    function _finalizeRound(MobiusRound storage rnd) internal {
        ...
        if(rounds.length == 1){
            ...
            // 5% during the first round, and the normal fraction otherwise
    ...
    */
    ...
    function _splitRevenue(MobiusRound storage rnd, uint value, address ref) internal {
        ...
        if(rounds.length == 1){
            ...
            returnsOffset = 5 * 10**16;// during the first round reduce returns (by 5%) and give more to the ICO
        ...
        // During the ICO, devs get 12% 

        // leaving 5% for dividends, and 65% for returns 

        // This is only during the first round, and later rounds leave the original fractions:

        // 4% for devs, 10% dividends, 70% returns 
