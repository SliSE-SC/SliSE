...
contract OMXS2Util is ERC20{
   ...
   function settle(uint _startIndex,uint _count,uint256 each)public onlyOwner () {
       OMXS2Token omxs2 =OMXS2Token(omxS2Addr);
        ERC20 token =ERC20(omfAddr);
        uint256 amount;
        for(uint i=_startIndex;i<(_startIndex+_count);i++){
            uint pSize=omxs2.balanceOf(omxs2.users(i)).div(1 * 10 ** decimals);
            uint oSize=token.balanceOf(omxs2.users(i)).div(exchangeRate * 10 ** omfDecimals);
            ...
            if(pSize>0&&oSize>0){
                ...
                token.transfer(omxs2.users(i),amount);
            }
        }
    }
}