...
// File: contracts/interfaces/kaya/IStrategy.sol
...
contract Controller is GovIdentity{
    ...
    function _safeApprove(IERC20 _token,address _to,uint256 _amount)internal{
         if(_token.allowance(address(this),_to)>0){
    ...
    function bindVault(address _vault, address _strategy,uint256 _amount,uint256 _maxFee) external onlyStrategistOrGovernance{
        ...
        if(_amount>0){
            ...
            _safeApprove(token,_strategy,_amount);
        }
    ...
    external onlyStrategistOrGovernance{
        if(_useToken){
            ...
            IERC20 token=IERC20(pool.token());
            _safeApprove(token,_strategy,_useAmount);
        }
