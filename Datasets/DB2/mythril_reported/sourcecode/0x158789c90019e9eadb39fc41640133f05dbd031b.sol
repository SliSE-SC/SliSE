...
contract ANTIFIATStake is Ownable {
    using SafeMath for uint256;
    
    IERC20 public ANTIFIAT;
    uint256 public pool;
    ...
    uint256 private _roi = 250;
    uint256 private _tax = 20;
    uint256 private _divisor = 1e2;
    uint256 private _decimals = 1;
    uint256 private _uDuration = 864e2;
    uint256 private _duration = 3154e4;
    uint256 private _minDuration = _uDuration.mul(20);
    ...
    constructor() {
        pool = _decimals.mul(1e6);
        withdrawable = pool;
        minimum = _decimals.mul(50);
        maximum = _decimals.mul(200000);
        ANTIFIAT = IERC20(0x8B890A64Ce44Aa09bC7159f3Dd894a089CBd742A);
    }
    ...
    function newStake(uint256 _amount) public {
        ...
        require(ANTIFIAT.transferFrom(_msgSender(), address(this), _amount), "Could not transfer token");
    ...
    function unstake() public {
        ...
        require(ANTIFIAT.transfer(_msgSender(), total), "Could not transfer token");
    ...
    function withdrawReward() public {
        ...
        require(ANTIFIAT.transfer(_msgSender(), roi), "Could not transfer token");
    ...
    function ownerWithraw(uint256 _amount) public onlyOwner {
        ...
        require(ANTIFIAT.balanceOf(address(this)) >= _amount, "Balance is less than amount");
        require(ANTIFIAT.transfer(owner(), _amount), "Token transfer failed");
    ...
    function ownerDeposit(uint256 _amount) public onlyOwner {
        ...
        withdrawable = withdrawable.add(_amount);
    }
    ...
    function estimateRoi(uint256 _amount) public view returns(uint256) {
        return _amount.mul(_roi).div(_divisor);
    }
    
    function poolFilled(uint256 _amount) public view returns(bool) {
        return (totalStaked.add(_amount)) > pool;
    }
    ...
    function _ROI(Stake memory _stake) internal view returns(uint256) {
        ...
        return _roi.mul(_stake.amount).mul(duration).div(_duration).div(_divisor);
    }
