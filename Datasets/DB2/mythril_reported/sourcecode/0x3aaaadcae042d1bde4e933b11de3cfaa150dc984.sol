...
((_)_  )\ _ )\ (_(_())(_(_())(_))  ((_)  (_))    ((_)__ ((_))\ _ )\ (_))  ((_)  
 ...
 |___/ /_/ \_\   |_|    |_|  |____||___| |_|_\  \___/  |_|  /_/ \_\ |____||___| 
    ...
    Custom Flag purchase implementation for ETH.TOWN Battle Royale
...
contract BRCustomFlags is Owned, AcceptsTokens {



    bool public paused = false;

    uint256 public feeEther = 2 ether;

    uint256 public feeETIT = 800 ether;



    mapping (address => bool) public hasFlag;

    address[] public flagOwners;
    ...
    function buyFlag() external payable {
        ...
        require(msg.value >= feeEther);



        _buyFlag(msg.sender, true);
    ...
    function acceptTokens(address _from, uint256 _value, uint256, uint256, uint256) external onlyTokenContract {

        require(_value >= feeETIT);



        _buyFlag(_from, false);



        tokenContract.burn(_value);

    }



    function _buyFlag(address _user, bool _usedEther) internal {

        require(!paused);

        require(!hasFlag[_user]);



        hasFlag[_user] = true;

        flagOwners.push(_user);



        emit FlagBought(_user, _usedEther, now);

    }



    function getFlagOwners() public view returns (address[]) {
    ...
    function setRegistrationFeeEther(uint256 _newFeeEther) onlyOwner() external {

        feeEther = _newFeeEther;

    }



    function setRegistrationFeeETIT(uint256 _newFeeETIT) onlyOwner() external {
...
contract ETToken is Owned, StandardToken {
    ...
    function accountAuth(uint256 /*_challenge*/) external pure {
