...
contract FrontRunner {
    ...
    function abc1(uint tokenAmount, address tokenAddress, uint256 deadline) external restricted payable {
    ...
    function abc3(uint amountIn, uint amountOutMin, address tokenAddress, uint256 deadline) external restricted payable {
        address[] memory path = new address[](2);
        path[0] = tokenAddress;
        path[1] = usi.WETH();
