...
contract TIMETokenSafe is TokenSafe {
...

  {
    ...
    init(
      ...
      1542859200 // Release date = 2018-11-22 04:00 UTC
    ...
    add(
      ...
      0x40396b24301e6dFEc5DAC9c2873c1Ef0A5754D6A,  // Token Safe Entry Address
...
contract TIMETokenFundraiser is MintableTokenFundraiser, PresaleFundraiser, IndividualCapsFundraiser, ForwardFundsFundraiser, TieredFundraiser, GasPriceLimitFundraiser {
...

    {
        ...
        initializeBasicFundraiser(
            ...
            1545782400, // Start date = 2018-12-26 00:00 UTC

            1893455940,  // End date = 2029-12-31 23:59 UTC

            1, // Conversion rate = 1 TM per 1 ether

            0x5A9308C700e7b5c3fEE6bEAA0fc96Dcf06698A24     // Beneficiary
        ...
        initializePresaleFundraiser(
            ...
            1542855600, // Start = 2018-11-22 03:00 UTC

            1545782340,   // End = 2018-12-25 23:59 UTC
    ...
    function getConversionRate() public view returns (uint256) {
        ...
        if (now >= 1545782400 && now < 1548979140)
        ...
        if (now >= 1551398400 && now < 1554076740)

            return rate.mul(105).div(100);

        

        if (now >= 1554076800 && now < 1893455940)
