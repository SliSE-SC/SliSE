...
// universal-protocol.com

// https://discord.gg/nUPnDpR
...
// this enables uniswap liquidity
interface IUniswapV2Pair { 
  function sync() external;
}


contract UniversalProtocol is ERC20 {
  using SafeMath for uint256;

  event PoolBurn(uint256 value); // decentralized liquidty

  mapping (address => uint256) private balances;
  mapping (address => mapping (address => uint256)) private allowed;
  string public constant name  = "UniversalProtocol";
  ...
  uint256 day = 86400; // 86400 seconds in one day, this is for burn function
  ...
  constructor() public {
      owner = msg.sender;
      balances[msg.sender] = _totalSupply; // owner of this smart contract mints total supply, these will all be added to uniswap liqudity pool
  }

  function totalSupply() public view returns (uint256) {
    return _totalSupply; // check total current cupply
  }

  function balanceOf(address addr) public view returns (uint256) {
    return balances[addr]; // check any specific wallets current UP token balance
  }

  function allowance(address addr, address spender) public view returns (uint256) {
    return allowed[addr][spender]; // allows tokens to be transfered to and from your address
  }
  ...
  function setPool(address _addr) public {
    require(msg.sender == owner); // only contract owner can set initial liquidty
    require(poolAddr == address(0)); // cannot be removed once liquidity is set
    poolAddr = _addr;
    lastBurnTime = now; // burning starts here
  }
