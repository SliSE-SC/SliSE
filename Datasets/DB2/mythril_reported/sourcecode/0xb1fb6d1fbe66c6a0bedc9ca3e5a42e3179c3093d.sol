...
contract VetteProtocol is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Vette Protocol";
    string private _symbol = "Vette";
    ...
    uint256 internal _tokenTotal = 10_000e18;
    ...
    uint256 public lastBuyback = now ;
    uint256 public buybackInterval = 30 minutes;
    ...
    event SwapedEthForTokens(uint256 EthAmount, uint256 TokenAmount, uint256 AmountBurned);
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "ERC20: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'ERC20: The contract it self cannot be excluded');
    ...
    function _transfer(
        ...
        //@dev Limit the transfer to 600 tokens for 6 minutes
        require(now > tradingEnabledAt + 6 minutes  || amount <= 5_000_000e18, "You cannot transfer more than 5 million coins.");
        ...
        if(!inSwapAndLiquify && sender != uniswapV2Pair) {
            ...
            //@dev Buy token
            if(now > lastBuyback + buybackInterval 
                && buybackEnabled
        ...
        if(!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && !inSwapAndLiquify){
            transferAmount = collectFee(sender,amount,rate, recipient == uniswapV2Pair);
        }
    ...
    function collectFee(address account, uint256 amount, uint256 rate, bool isSell) private returns (uint256) {
        uint256 transferAmount = amount;
        ...
        if(_buybackFee != 0){
            ...
            _buybackTotal = _buybackTotal.add(buybackFee);
            emit Transfer(account,address(this),buybackFee);
        }
    ...
    function buyAndBurnToken(uint256 contractBalance) private lockTheSwap {
        ...
        _burnFeeTotal = _burnFeeTotal.add(swapedTokens);
        _tokenTotal = _tokenTotal.sub(swapedTokens);
        _reflectionTotal = _reflectionTotal.sub(swapedTokens.mul(rate));

        emit Transfer(address(balancer), address(0), swapedTokens);
        emit SwapedEthForTokens(contractBalance, swapedTokens, swapedTokens);
    }
    ...
    function setBuybackFee(uint256 fee) public onlyOwner {
        _buybackFee = fee;
    }
