...
// File: contracts/TrenderingAIMv0.sol

pragma solidity ^0.6.0;

/**
 * @title Trendering AIM v0
 * @author C based on source by https://github.com/vittominacori
 * @dev Implementation of the Trendering AIM v0
 */
contract TrenderingAIMv0 is ERC20Burnable, Roles, TokenRecover {
    using SafeMath for uint256;
    using Address for address;
    ...
    constructor(
        ...
        TRND_requirement = TRND_requirement.mul(1e18);

        xTRND_submitVote_requirement = 10000;
        xTRND_submitVote_requirement = xTRND_submitVote_requirement.mul(1e18);

        xTRND_endVote_bonus = 500;
        xTRND_endVote_bonus = xTRND_endVote_bonus.mul(1e18);
    ...
    function gongi() public onlyOwner {
        DAI_debt = DAI_token.balanceOf(address(this));
    ...
    function deposit(uint256 _amount) public {
        ...
        require(TRND_token.balanceOf(address(msg.sender)) >= TRND_requirement, "TRND requirement not satisfied.");
    ...
    function withdraw(uint256 _amount) public {
        ...
        uint256 DAI_share = _amount.mul(DAI_total).div(totalSupply());
