...
contract BancorV2_ZapIn_General_V2 is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using Address for address;
        ...
        returns (uint256 lptReceived, uint256 id)
    {
        ...
        address poolAnchor = IBancorV2Converter(_toBanConverterAddress)
            .anchor();
        uint256 initialVbntBalance = vBNTToken.balanceOf(address(this));
        ...
        uint256 finalVbntBalance = vBNTToken.balanceOf(address(this)).sub(
            initialVbntBalance
        );

        if (finalVbntBalance > 0)
            vBNTToken.safeTransfer(msg.sender, finalVbntBalance);

        emit Zapin(msg.sender, poolAnchor, lptReceived, finalVbntBalance, id);
    }
    ...
    function _enterBancor(
        ...
        id = bancorLiquidityProtection.addLiquidity.value(valueToSend)(
            _poolAnchor,
            reserveToken,
            _amount
        );
        ...
        id = bancorLiquidityProtection.transferLiquidity(id, msg.sender);
    }
