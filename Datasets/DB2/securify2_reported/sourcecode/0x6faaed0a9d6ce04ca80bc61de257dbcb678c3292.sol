...
// File contracts/assets/AirdropWithdraw.sol
...
contract AirdropWithdraw is Ownable {
    ...
    function setClaimEqual(address[] calldata _claimers, uint256 balance) public onlyOwner {
            uint8 i = 0;
            for (i; i < _claimers.length; i++) {
                rewardBalance[_claimers[i]] = balance;
                requiredBalance = requiredBalance.add(balance);
            }
    ...
    function setClaim(address[] calldata _claimers, uint256[] calldata balances) public onlyOwner {
            uint8 i = 0;
            for (i; i < _claimers.length; i++) {
                rewardBalance[_claimers[i]] = balances[i];
                requiredBalance = requiredBalance.add(balances[i]);
            }
    }
    function withdraw() public {
            uint amount = rewardBalance[msg.sender];
            require(amount > 0, "No reawrd");
            
            uint256 tokenBalance = rewardToken.balanceOf(address(this));
            require(tokenBalance > 0, "No reward in contract");

            if (amount > tokenBalance) {
                rewardBalance[msg.sender]  = rewardBalance[msg.sender].sub(tokenBalance);
