...
contract HOStaking {
    ...
    function wdV2Earn() public {
      ...
      IERC20(hoToken).transfer(msg.sender, earnsTotal.sub(fee));
    ...
    function wdHo(uint amount) external {
      ...
      if(stakingToal - amount >= LEAST * 10 ** uint(hoDec)) {
        ...
        IERC20(hoToken).transfer(msg.sender, amount.sub(fee));
        ...
        IERC20(hoToken).transfer(msg.sender, stakingToal.sub(fee));
