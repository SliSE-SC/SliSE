...
contract ChonkTokenWrapper {
	using SafeMath for uint256;
	IERC20 public chonk;

	constructor(IERC20 _chonkAddress) public {
		chonk = IERC20(_chonkAddress);
	}
	...
	function stake(uint256 amount) public {
		...
		chonk.transferFrom(msg.sender, address(this), amount);
	}

	function withdraw(uint256 amount) public {
		_totalSupply = _totalSupply.sub(amount);
		_balances[msg.sender] = _balances[msg.sender].sub(amount);
		chonk.transfer(msg.sender, amount);
	}
}

contract GenesisPool is ChonkTokenWrapper, Ownable {
	...
	constructor(ERC1155Tradable _chonkersAddress, IERC20 _chonkAddress) public ChonkTokenWrapper(_chonkAddress) {
	...
	// stake visibility is public as overriding ChonkTokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		require(amount.add(balanceOf(msg.sender)) <= 5000000000000000000, "Cannot stake more than 5 chonk");
		super.stake(amount);
