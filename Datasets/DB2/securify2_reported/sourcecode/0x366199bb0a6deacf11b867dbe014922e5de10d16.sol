...
contract ERC20Migrator is Ownable {
     ...
     */
    constructor(ERC20Detailed legacyToken) public {
        require(
            address(legacyToken) != address(0),
            "ERC20Migrator: legacy token is the zero address"
        );
        require(
            bytes(legacyToken.symbol()).length > 0,
     ...
     */
    function beginMigration(ERC20Mintable newToken_) external onlyOwner returns (bool) {
