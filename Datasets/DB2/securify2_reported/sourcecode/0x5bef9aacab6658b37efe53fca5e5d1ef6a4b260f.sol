...
// File: contracts/TokenPrintRegistry.sol

pragma solidity ^0.5.0;

contract TokenPrintRegistry is CustomERC721Metadata, WhitelistedRole, CryptoPunksMarket {
    using SafeMath for uint256;
    ...
    CryptoPunksMarket c = CryptoPunksMarket(0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB);
    ...
    constructor(address payable _printerAddress, uint256 _pricePerPrintInWei, uint256 _pricePerNFCInWei, string memory _tokenBaseURI, string memory _tokenName, string memory _tokenSymbol) CustomERC721Metadata(_tokenName, _tokenSymbol) public {
    ...
    function purchasePrintWithArtId(uint256 _artId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
        return purchasePrintWithArtIdTo(msg.sender, _artId, _contactMethod);
    }

    function purchasePrintWithArtIdTo(address _to, uint256 _artId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
        require(msg.value == pricePerPrintInWei, "Must send pricePerPrintInWei");
       
        require(msg.sender == c.punkIndexToAddress(_artId), "You must own punk");
     ...
     function purchaseNFCWithArtId(uint256 _artId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
        return purchaseNFCWithArtIdTo(msg.sender, _artId, _contactMethod);
    }

    function purchaseNFCWithArtIdTo(address _to, uint256 _artId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
        require(msg.value == pricePerNFCInWei, "Must send pricePerNFCInWei");
    ...
    function mintWithArtIdAndNFCTo(address _to, uint256 _artId, string memory _nfc) public onlyWhitelisted returns (uint256 _tokenId) {
    ...
    function updateTokenArtId(uint256 _tokenId, uint256 _artId) public onlyWhitelisted returns (bool) {
