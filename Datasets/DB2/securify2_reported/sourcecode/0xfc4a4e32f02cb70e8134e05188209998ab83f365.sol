...
contract ALBToken is ERC20Meta, ERC20Detailed, Ownable {

    using SafeERC20 for ERC20;

	

    mapping (address => uint256) beneficiaries_now;

    mapping (address => uint256) beneficiaries_1year;

    mapping (address => uint256) beneficiaries_2year; 



    uint256 stopdate_1year = now + 365 * 1 days;

    uint256 stopdate_2year = now + 730 * 1 days;



    address beneficiary1 = 0x188f445c13F702cBB3734Df63fb4Cb96c474866d;

    address beneficiary2 = 0x11308512672859E403C570996dA51EEb6F5d1cD3;

    address beneficiary3 = 0xc643E129c49CAb15dfC964643f2deCC9Dbfc7091;

    address beneficiary4 = 0x3c35a288d9EA25E8F727C7d45383c04E633B5bdC;
    ...
    constructor() public ERC20Detailed("AllBeBet", "ALB", 0) {
        ...
        stopdate_1year = now + 365 * 1 days;

        stopdate_2year = now + 730 * 1 days;
        ...
        beneficiaries_now[beneficiary1] = amount.mul(20).div(100);

        beneficiaries_1year[beneficiary2] = amount.mul(30).div(100);

        beneficiaries_1year[beneficiary3] = amount.mul(75).div(1000);

        beneficiaries_1year[beneficiary4] = amount.mul(25).div(1000);

        beneficiaries_2year[beneficiary2] = amount.mul(30).div(100);

        beneficiaries_2year[beneficiary3] = amount.mul(75).div(1000);

        beneficiaries_2year[beneficiary4] = amount.mul(25).div(1000);
    ...
    function release() public {
        ...
        if (beneficiaries_now[msg.sender] > 0) {

            IERC20(address(this)).transfer(msg.sender, beneficiaries_now[msg.sender]);

            beneficiaries_now[msg.sender] = 0;

        }

        if (now >= stopdate_1year && beneficiaries_1year[msg.sender] > 0) {

            IERC20(address(this)).transfer(msg.sender, beneficiaries_1year[msg.sender]);

            beneficiaries_1year[msg.sender] = 0;

        }

        if (now >= stopdate_2year && beneficiaries_2year[msg.sender] > 0) {

            IERC20(address(this)).transfer(msg.sender, beneficiaries_2year[msg.sender]);

            beneficiaries_2year[msg.sender] = 0;
