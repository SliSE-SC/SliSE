...
contract SalePool is TokenWrapper, Ownable {
  ...
  constructor(ERC1155Tradable _nftsAddress, IERC20 _tokenAddress, ISalePoolUtils _utils) public TokenWrapper(_tokenAddress) {
    nfts = _nftsAddress;
    utils = _utils;
  }
  ...
  function earned(address account) public view returns (uint256) {
    ...
    return points[account].add(blockTime.sub(lastUpdateTime[account]).mul(utils.getProduction(balanceOf(account))));
  }
