...
// File: contracts/711/dependencies/IRewardsDistributor.sol
...
interface IRewardsDistributor {
    ...
    function distribute(bool active) external;
    function addPendingRewards(uint256 amount) external;
    function isNotifiable() external view returns (bool);
}
...
// File: contracts/711/RewardsDistributor.sol
...
contract RewardsDistributor is Ownable, IRewardsDistributor, ADDRESSBOOK {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 constant public UNIT = 4.75247525e18;
    uint256 constant public TOTAL_ROUND = 100;
    ...
    function distribute(bool active) public {
        ...
        pendingRewards = 0;
        if(active) {
            uint256 rewardForNotifier = reward.mul(NOTIFY_MULTIPLIER).div(10000);
    ...
    function isNotifiable() public view returns (bool) {
        return now > lastDistributeTime.add(INTERVAL);
    }
