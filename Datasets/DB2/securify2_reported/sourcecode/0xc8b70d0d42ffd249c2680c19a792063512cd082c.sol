...
contract DRFTokenWrapper {
    ...
    constructor(IERC20 _drfAddress) public {
...
contract GenesisPool is DRFTokenWrapper, Ownable {
    ERC1155Tradable public drfs;
    ...
    constructor(ERC1155Tradable _drfsAddress, IERC20 _drfAddress) public DRFTokenWrapper(_drfAddress) {
        drfs = _drfsAddress;
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return
        points[account].add(
            blockTime.sub(lastUpdateTime[account]).mul(1e18).div(86400).mul(balanceOf(account).div(1e18))
        );
    }

    // stake visibility is public as overriding DRFTokenWrapper's stake() function
    function stake(uint256 amount) public updateReward(msg.sender) {
        require(amount.add(balanceOf(msg.sender)) <= 100000000000000000000, "Cannot stake more than 100 DRF");
    ...
    function redeem(uint256 card) public updateReward(msg.sender) {
        ...
        require(drfs.totalSupply(card) < drfs.maxSupply(card), "Max cards minted");

        points[msg.sender] = points[msg.sender].sub(cards[card]);
        drfs.mint(msg.sender, card, 1, "");
