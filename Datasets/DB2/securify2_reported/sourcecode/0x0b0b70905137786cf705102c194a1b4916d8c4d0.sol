...
contract CANVAS is ERC721Full {
    ...
    mapping(uint256 => tokenComment[]) public tokenComments;
    ...
    constructor() public ERC721Full("cryptocanvas.xyz", "CANVAS") {
        // Data string ID 0 -> non transferable and no owner
        tokenStringData.push("");
        owner = msg.sender;

        _setBaseURI("https://cryptocanvas.xyz/api/token/");
    }
    ...
    function mint(string memory newTokenString) public payable {
        ...
        _setTokenURI(_id - 1, Strings.fromUint256(_id - 1));
    }
    ...
    /// @notice Returns a list of all CANVAS Token IDs assigned to an address.
    /// @param _owner The owner whose CANVAS Tokens we are interested in.
    /// @dev This method MUST NEVER be called by smart contract code. First, it's fairly
    ///  expensive (it walks the entire CANVAS tokens array looking for tokens belonging to owner),
        ...
        returns (uint256[] memory ownerTokens)
    {
        ...
        if (tokenCount == 0) {
            ...
            uint256 totalCANVAS = totalSupply();
            uint256 resultIndex = 0;

            // We count on the fact that all CANVAS tokens have IDs starting at 1 and increasing
            // sequentially up to the totalCANVAS count.
            uint256 tokenId;

            for (tokenId = 1; tokenId <= totalCANVAS; tokenId++) {
