...
contract StakeContract is Ownable, StakingInterface {
    ...
    constructor(address _ethy) public {
        ethy = IERC20(_ethy);
    }
    ...
    function stake(uint256 _amount, uint256 _days) public {
        ...
        require(ethy.balanceOf(address(this)) > totalOwedValue().add(_reward).add(_amount), "insufficient contract bal");

        require(ethy.transferFrom(msg.sender, address(this), _amount), "transfer failed");
    ...
    function claim() public {
        ...
        require(ethy.transfer(msg.sender, owed), "transfer failed");
    }
    ...
    function ownerReclaim(uint256 _amount) public onlyOwner {
        require(_amount < ethy.balanceOf(address(this)).sub(totalOwedValue()), "cannot withdraw owed funds");
        ethy.transfer(msg.sender, _amount);
    }
