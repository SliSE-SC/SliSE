...
contract LPTokenWrapper {
    ...
    IERC20 public y = IERC20(0xC027B0aaDb022fc7c97923398E3A2894c5957506);  //define uni token
...
contract MORBPool is LPTokenWrapper, IRewardDistributionRecipient {
    IERC20 public morb = IERC20(0xee76c208fD0A57C2789863A76FA8532c06968265);    //define token

    uint256 public  DURATION = 4 weeks;
    uint256 public  NUMBER_EPOCHS = 100000000;

    uint256 public  EPOCH_REWARD = 50 ether;
    ...
    uint256 public starttime = 1602161179;
    ...
    function earned(address account) public view returns (uint256) {
        uint256 calculatedEarned = balanceOf(account)
            ...
            .div(1e18)
            .add(rewards[account]);
        uint256 poolBalance = morb.balanceOf(address(this));
    ...
    function setRewardStake(address _rewardStake) external onlyOwner {
        ...
        morb.approve(rewardStake, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
    }

    function stake(uint256 amount) public updateReward(msg.sender) checkNextEpoch checkStart {
        require(amount > 0, "Cannot stake 0");
        timeUnlock[msg.sender]= durationForUnlock + block.timestamp;
    ...
    function getReward() public updateReward(msg.sender) checkNextEpoch checkStart returns (uint256) {
        require(block.timestamp > timeUnlock[msg.sender], "Can't claim rewards, time for unlock hasn't passed");
        uint256 reward = earned(msg.sender);
        if (reward > 1) {
            accumulatedStakingPower[msg.sender] = accumulatedStakingPower[msg.sender].add(rewards[msg.sender]);
            rewards[msg.sender] = 0;

            morb.safeTransfer(msg.sender, reward);
    ...
    function nextRewardMultiplier() public view returns (uint16) {
        if (rewardVote != address(0)) {
            uint16 votingValue = MORBVote(rewardVote).averageVotingValue(address(this), periodFinish);
    ...
    modifier checkNextEpoch() {
        if (block.timestamp >= periodFinish) {
            ...
            if (currentEpoch==2){
                NUMBER_EPOCHS= 10000;
                EPOCH_REWARD= 10 ether;
                DURATION= 26 weeks;
                currentEpochReward = EPOCH_REWARD;
            }
            
            if (currentEpochReward > 0) {
                morb.mint(address(this), currentEpochReward);
