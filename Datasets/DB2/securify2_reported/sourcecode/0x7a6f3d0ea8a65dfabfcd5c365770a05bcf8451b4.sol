...
// File: contracts/libraries/nonReentrancy.sol
...
// File: contracts/multi_sender.sol

// SPDX-License-Identifier: Leafan
...
contract MultiSender is ReentrancyGuard {
    ...
    function multisendToken(address token, address payable[] memory _receivers, uint256[] memory _balances) public payable nonReentrant {
        if (token == ETH_ADDRESS){
            multisendEther(_receivers, _balances);
        } else {
            uint256 total = 0;

            require(msg.value >= currentFee(), "fee is too less.");
