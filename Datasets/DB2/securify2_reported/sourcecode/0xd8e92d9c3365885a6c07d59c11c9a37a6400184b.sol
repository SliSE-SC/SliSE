...
interface INFT {
    function mintTo(address account) external;

    function multiMint(address account, uint256 amount) external;

    function successIphoneMade() external view returns (uint256);

    function createdIphoneMade() external view returns (uint256);
