...
from executor.utils import get_argument_value, is_symbolic
from finder import vulnerability_finder
from finder.vulnerability import Vulnerability
...
def __reentrancy_pre_call(analyzed_block, sstores, call_value):
    ...
    if is_symbolic(call_value):
        print("call_value.num_args()",call_value.num_args())
...
def __reentrancy_pre_call1(analyzed_block, sstores, call_value, storage_value):
    constraints = []
    # Check if guard is not detected
    for constraint in analyzed_block.constraints:
        constraint = simplify(constraint)
    ...
    storage_call = __get_storage_var(str(storage_value), analyzed_block.state, sstores)

    # Check if value of storage is different than 0
    if storage_call is None:
        constraints.append(True)
        return True
        # return constraints
    else :
        if is_symbolic(call_value):
            ...
            # constraints.append(storage_call != 0)
            return constraints
...
def __get_block_with_constraint(constraint, trace):
    ...
    for block in reversed(trace.analyzed_blocks):
        if found and block.constraints:
            if hash(constraint) != hash(block.constraints[-1]):
                return block
            ...
            for instruction in analyzed_block.block.insns:
                ...
                if constraints:
                    ...
                    if __has_vulnerability(constraints):
                        ...
                        print("__reentrancy_pos_call",vuln.line_number)
                        all_vulns.add(vuln)
                        if not find_all:
                            return all_vulns
                    continue
                ...
                storage_value = get_argument_value(instruction.arguments, 1, analyzed_block.state.registers)
                

                
                # # if is_symbolic(call_value):
                # if __get_storage_var(str(storage_value), analyzed_block.state, sstores) is None:
                # print("is_symbolic(call_value)",is_symbolic(call_value))
                # print("storage_value",storage_value)
                # print("__get_storage_var(str(storage_value), analyzed_block.state, sstores)",__get_storage_var(str(storage_value), analyzed_block.state, sstores))
                # print("storage_value.num_args()",storage_value.num_args())
                    # import pdb
                    # pdb.set_trace()
                
                # constraints = __reentrancy_pre_call(analyzed_block, sstores, call_value)
                constraints = __reentrancy_pre_call1(analyzed_block, sstores, call_value, storage_value)
                if constraints:
                    exist_constraints = True
                    if __has_vulnerability(constraints):
                        vuln = Vulnerability(REENTRANCY_TYPE, analyzed_block, offset, instruction_offset)
