...
class InternalRecover(object):
        ...
        for block in function:
            for insn in list(block.insns):  # make a new list because we modify it during iterating
                ...
                if insn.insn.is_push:
                    ...
                    # print("ConcreteStackValue(insn.insn.operand, self.concrete_values)",insn.insn.operand, self.concrete_values)
    ...
    def identify_blocks(self, function: SSAFunction) -> None:
        ...
        hex_data = re.findall(b'[0-9a-fA-F]+', self.filedata)
        self.filedata = b''.join(hex_data)

        # print("self.filedata",type(self.filedata))
...
class Recover1(object):
    internal: InternalRecover1

    def __init__(self, filedata: bytes, edges: List[Tuple[int, int]], optimize=False, split_functions=True) -> None:
        self.internal = InternalRecover1(filedata, edges, optimize, split_functions)
