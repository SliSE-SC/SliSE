...
class StackValue(object):
    ...
    def resolve(self, visited=False) -> Tuple['StackValue', bool]:
        ...
        if len(self.block.in_edges) > 1:
            ...
            for edge in self.block.in_edges:
                ...
                try:
                    ...
                    new_slot, _ = new_slot.resolve()  # 尽可能将其解析
                except IndexError:
                    '''
                    父块没有足够的栈空间，我猜应该将其上移调用堆栈，
                    但我这里不这样做。
                    '''
                    # new_slot = PlaceholderStackValue(edge, self.sp + len(edge_stack))  # 创建一个新的占位符，表示未解决的值
...
class SSABasicBlock(object):
    ...
    #     if other not in self.function.blockmap.keys():
    #         target_block = self.function.blockmap[list(self.function.blockmap.keys())[-1]]
    #     else:
    #         target_block = self.function.blockmap[other]
